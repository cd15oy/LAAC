...
class Characterizer:
    #fixedDimensionality: False if problem dimensionality of problems is not fixed 
    #dimensionality: dimensionality of the problems (if it is fixed) 
    def __init__(self, fixedDimensionality:bool = False, dimensionality:int=None):
        self.dimensionality = dimensionality
        self.fixedDimensionality = fixedDimensionality
        self.characterizer = cdll.LoadLibrary('FLA/Characterize.so')

    #returns the size of feature vectors
    def featureSize(self) -> int:
        if self.fixedDimensionality:
            return 79 + (4*self.dimensionality)
        else:
            return 79
